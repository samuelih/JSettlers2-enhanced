---
id: INV-CL-009
name: single-writer-viewport-scrolltop
type: INVARIANT
status: candidate
...
checks:
  - oracle: code.search
    query: assignments to `scrollTop` (`*.scrollTop = ...`)
    scope: web
...
defined-in: invariants/INV-CL-009.invariant.md
references:
...
# [INV-CL-009] single-writer: field `viewport.scrollTop` is mutated by exactly one function (`MapEditorScreen`)

> _Detected by ; HYPOTHESIS candidate invariant pending human ratification._

## Guarantee
single-writer: field `viewport.scrollTop` is mutated by exactly one function (`MapEditorScreen`)
...
## Checks
- `code.search` — query: `assignments to \`scrollTop\` (\`*.scrollTop = ...\`)` (scope: `web`); expected: every assignment site resolves into the single writer function named in this record (2 cited site(s)); a new writer elsewhere is a drift signal

## Cited By Code
- [web/src/screens/MapEditorScreen.tsx:244](../../../web/src/screens/MapEditorScreen.tsx#L244) — `viewport.scrollTop = ...`
- [web/src/screens/MapEditorScreen.tsx:279](../../../web/src/screens/MapEditorScreen.tsx#L279) — `viewport.scrollTop = ...`

## Forbidden Bypasses
- Direct writes to `viewport.scrollTop` outside the cited writer function.
