---
id: INV-CL-003
name: single-writer-parsed-boolvalue
type: INVARIANT
status: candidate
...
checks:
  - oracle: code.search
    query: assignments to `boolValue` (`*.boolValue = ...`)
    scope: web
...
defined-in: invariants/INV-CL-003.invariant.md
references:
...
# [INV-CL-003] single-writer: field `parsed.boolValue` is mutated by exactly one function (`parseOptions`)

> _Detected by ; HYPOTHESIS candidate invariant pending human ratification._

## Guarantee
single-writer: field `parsed.boolValue` is mutated by exactly one function (`parseOptions`)
...
## Checks
- `code.search` — query: `assignments to \`boolValue\` (\`*.boolValue = ...\`)` (scope: `web`); expected: every assignment site resolves into the single writer function named in this record (2 cited site(s)); a new writer elsewhere is a drift signal

## Cited By Code
- [web/src/protocol/gameOptions.ts:317](../../../web/src/protocol/gameOptions.ts#L317) — `parsed.boolValue = ...`
- [web/src/protocol/gameOptions.ts:320](../../../web/src/protocol/gameOptions.ts#L320) — `parsed.boolValue = ...`

## Forbidden Bypasses
- Direct writes to `parsed.boolValue` outside the cited writer function.
