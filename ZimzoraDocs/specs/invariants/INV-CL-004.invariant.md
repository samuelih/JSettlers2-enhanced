---
id: INV-CL-004
name: single-writer-parsed-intvalue
type: INVARIANT
status: candidate
...
checks:
  - oracle: code.search
    query: assignments to `intValue` (`*.intValue = ...`)
    scope: web
...
defined-in: invariants/INV-CL-004.invariant.md
references:
...
# [INV-CL-004] single-writer: field `parsed.intValue` is mutated by exactly one function (`parseOptions`)

> _Detected by ; HYPOTHESIS candidate invariant pending human ratification._

## Guarantee
single-writer: field `parsed.intValue` is mutated by exactly one function (`parseOptions`)
...
## Checks
- `code.search` — query: `assignments to \`intValue\` (\`*.intValue = ...\`)` (scope: `web`); expected: every assignment site resolves into the single writer function named in this record (2 cited site(s)); a new writer elsewhere is a drift signal

## Cited By Code
- [web/src/protocol/gameOptions.ts:321](../../../web/src/protocol/gameOptions.ts#L321) — `parsed.intValue = ...`
- [web/src/protocol/gameOptions.ts:323](../../../web/src/protocol/gameOptions.ts#L323) — `parsed.intValue = ...`

## Forbidden Bypasses
- Direct writes to `parsed.intValue` outside the cited writer function.
