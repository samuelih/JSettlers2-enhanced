---
id: INV-CL-006
name: single-writer-prevpiececount-current
type: INVARIANT
status: candidate
...
checks:
  - oracle: code.search
    query: assignments to `current` (`*.current = ...`)
    scope: web
...
defined-in: invariants/INV-CL-006.invariant.md
references:
...
# [INV-CL-006] single-writer: field `prevPieceCount.current` is mutated by exactly one function (`useGameFeedback`)

> _Detected by ; HYPOTHESIS candidate invariant pending human ratification._

## Guarantee
single-writer: field `prevPieceCount.current` is mutated by exactly one function (`useGameFeedback`)
...
## Checks
- `code.search` — query: `assignments to \`current\` (\`*.current = ...\`)` (scope: `web`); expected: every assignment site resolves into the single writer function named in this record (2 cited site(s)); a new writer elsewhere is a drift signal

## Cited By Code
- [web/src/screens/GameScreen.tsx:871](../../../web/src/screens/GameScreen.tsx#L871) — `prevPieceCount.current = ...`
- [web/src/screens/GameScreen.tsx:887](../../../web/src/screens/GameScreen.tsx#L887) — `prevPieceCount.current = ...`

## Forbidden Bypasses
- Direct writes to `prevPieceCount.current` outside the cited writer function.
