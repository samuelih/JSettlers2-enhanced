...
- For developers:
	...
	- Cities & Knights: New playable scenario `SC_CK` (sea board, 13 VP to win), implementing Phases 1-3 of `doc/Cities-and-Knights-Design.md`
	    - Commodities (cloth/coin/paper) as per-player counters separate from the 5-resource set: cities on pasture/mountain/forest hexes yield 1 resource + 1 commodity instead of 2 resources
	    - City improvements: Trade/Politics/Science tracks built level 1-5 for commodities through the existing `SOCSetSpecialItem` PICK flow; first to level 4 claims that track's metropolis (+2 SVP, stealable by exceeding the holder's level)
	    - Knights as per-player leveled state (basic/strong/mighty, active/inactive): buy (1 sheep + 1 ore), activate (1 wheat), promote (1 sheep + 1 ore; mighty requires Politics level 3+); new `SOCSimpleRequest` types 1002-1004
	    - Barbarians: strength counter advances each roll and attacks at 7 against the players' total active knight strength; sole strongest defender becomes Defender of Catan (+1 SVP), or the weakest defenders each have a city downgraded to a settlement (`SOCRemovePiece(CITY)`)
	    - Progress cards: three 10-card decks (`SOCInventoryItem` itypes 11-19) drawn by improvement level on each roll, playable the same turn with no one-per-turn limit; includes Resource/Trade Monopoly, Master Merchant, Warlord, Wedding, Irrigation, Mining, and VP cards
	    - Built-in bots play the base game in `SC_CK` games and ignore the new mechanics without hanging
	    - The `_CK_*` game options are now active (set together by the scenario); `ckcomm:` debug command grants commodities
	    - Full rules, wire protocol, and simplifications vs. the boxed game: `doc/Cities-and-Knights-Implemented.md`; the formerly-PROPOSED message sequences in `doc/Message-Sequences-for-Game-Actions.md` are now implemented as documented there
	    - Bugfix found during implementation: large sea board generator could leave a desert hex holding a gold hex's dice number when separating adjacent gold hexes
