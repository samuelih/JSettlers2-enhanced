---
id: INV-CL-002
name: single-writer-desc-curstrvalue
type: INVARIANT
status: candidate
...
checks:
  - oracle: code.search
    query: assignments to `curStrValue` (`*.curStrValue = ...`)
    scope: web
...
cited-by-code:
  - oracle: code.search
    query: desc.curStrValue = ...
    scope: web/src/protocol
promotion-criteria:
...
defined-in: invariants/INV-CL-002.invariant.md
references:
...
# [INV-CL-002] single-writer: field `desc.curStrValue` is mutated by exactly one function (`descriptorFromInfo`)

> _Detected by ; HYPOTHESIS candidate invariant pending human ratification._

## Guarantee
single-writer: field `desc.curStrValue` is mutated by exactly one function (`descriptorFromInfo`)
...
- `web/src/protocol/gameOptions.ts`
...
## Checks
- `code.search` — query: `assignments to \`curStrValue\` (\`*.curStrValue = ...\`)` (scope: `web`); expected: every assignment site resolves into the single writer function named in this record (2 cited site(s)); a new writer elsewhere is a drift signal

## Cited By Code
- [web/src/protocol/gameOptions.ts:176](../../../web/src/protocol/gameOptions.ts#L176) — `desc.curStrValue = ...`
- [web/src/protocol/gameOptions.ts:178](../../../web/src/protocol/gameOptions.ts#L178) — `desc.curStrValue = ...`

## Forbidden Bypasses
- Direct writes to `desc.curStrValue` outside the cited writer function.
